...
describe("Chat User Journey", () => {
  ...
  describe("Manual itinerary generation", () => {
    // These tests and pending will be implemented when the manual generation feature is added
    it.todo("should generate itinerary directly when requested");
    it.todo("should handle errors during manual itinerary generation");
  });
  ...
  describe("Error handling and session recovery behaviors", () => {
    ...
    it("should reset the chat when clicking Start New Plan", async () => {
      // 1. Set up a session with existing messages
      useChatStore.setState({
        sessionId: mockSessionId,
        messages: [
      ...
      expect(screen.getByText("I want to go to Palawan")).toBeInTheDocument();
      expect(
        screen.getByText("I need a few more details to plan your perfect trip.")
      ).toBeInTheDocument();

      // 3. Find and click the Start New Plan button
      const startNewButton = screen.getByRole("button", {
        name: /start new plan/i,
      });
      await userEvent.setup().click(startNewButton);

      // 4. Verify confirm dialog was shown (if your app uses confirm)
      expect(window.confirm).toHaveBeenCalled();

      // 5. Verify chat is reset
      expect(
        screen.queryByText("I want to go to Palawan")
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(
          "I need a few more details to plan your perfect trip."
      ...
      // 6. Verify initial state is shown
      expect(screen.getByText("Try to say:")).toBeInTheDocument();
      expect(
        screen.getByText('"I want to visit Palawan for 3 days"')
      ).toBeInTheDocument();

      // 7. Verify store was reset
      expect(useChatStore.getState().sessionId).toBeNull();
