...
class ChatService:
    ...
    Coordinates intent extraction, clarification question flow, and itinerary
    generation using Redis-backed session management.
    """
    ...
    async def _schedule_cleanup(self, session_id: str) -> None:
        """Shorten session TTL after itinerary is complete."""
        try:
    ...
    async def process_message(
        ...
        Args:
            message: User's raw text message.
            session_id: Optional existing session ID to continue.
            user_id: Optional authenticated user ID.

        Returns:
            ``ClarificationResponse`` if more info is needed, otherwise
            ``ItineraryResponse`` with the generated trip.
        """
        try:
    ...
    async def process_clarification(
        ...
        Args:
            session_id: Session ID for which answers are being submitted.
            answers: Dict mapping question IDs to user-provided values.
            user_id: Optional authenticated user ID.

        Returns:
            Another ``ClarificationResponse`` if more info needed, or
            ``ItineraryResponse`` when intent is complete.
        """
        try:
            session = await self._get_or_create_session(session_id, user_id)
